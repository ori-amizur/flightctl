...
func NewCallbackManager(sender Sender, log logrus.FieldLogger) CallbackManager {
...
func (t *callbackManager) submitTask(taskName string, resource ResourceReference, op string) error {
	resource.TaskName = taskName
	resource.Op = op
	return t.sender.Send(&resource)
}
...
func DispatchCallbacks(store store.Store, callbackManager CallbackManager) ReceiveHandler {
	return func(ctx context.Context, reference *ResourceReference, log logrus.FieldLogger) error {
