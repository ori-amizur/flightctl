...
func (s *DeviceStore) Healthcheck(ctx context.Context, orgId uuid.UUID, names []string) error {
	result := s.getDB(ctx).Model(&model.Device{}).Where("org_id = ? and name in (?)", orgId, names).Updates(map[string]any{
		"last_seen":        time.Now(),
		"resource_version": gorm.Expr("resource_version + 1"),
	})
	if result.Error != nil {
	...
	s.log.Infof("Updated healthcheck for %d devices expected %d", result.RowsAffected, len(names))
	return nil
}
