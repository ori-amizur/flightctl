...
func (s *DeviceStore) get(ctx context.Context, orgId uuid.UUID, name string, opts ...model.APIResourceOption) (*api.Device, error) {
...
func (s *DeviceStore) GetWithoutServiceConditions(ctx context.Context, orgId uuid.UUID, name string) (*api.Device, error) {
	return s.get(ctx, orgId, name, model.WithoutServiceConditions())
}
...
func (s *DeviceStore) ListDisconnected(ctx context.Context, orgId uuid.UUID, listParams ListParams, cutoffTime time.Time) (*api.DeviceList, error) {
	...
	queryStr := `SELECT d.*, dt.last_seen
          FROM devices d, device_timestamps dt
          WHERE d.org_id = ? AND d.deleted_at is NULL AND d.org_id = dt.org_id AND dt.last_seen < ? `
	...
	ret, err := model.DevicesToApiResource(lo.Map(devices, func(d model.DeviceWithTimestamp, _ int) model.Device {
		return d.ToDeviceResource()
	}), nextContinue, numRemaining)
	if err != nil {
		return nil, err
	}
