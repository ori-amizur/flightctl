...
func (h *ServiceHandler) GetRenderedDevice(ctx context.Context, name string, params api.GetRenderedDeviceParams) (*api.Device, api.Status) {
	...
	if params.KnownRenderedVersion != nil {
		isNew, kvRenderedVersion, err = rendered_version.RenderedVersionManager.Instance().WaitForNewRenderedVersion(ctx, orgId, name, *params.KnownRenderedVersion)
		if err != nil {
	...
	if kvRenderedVersion != "" && newVersion != "" && kvRenderedVersion != newVersion {
		...
		if err = rendered_version.RenderedVersionManager.Instance().Store(ctx, orgId, name, newVersion); err != nil {
...
func (h *ServiceHandler) UpdateRenderedDevice(ctx context.Context, name, renderedConfig, renderedApplications string) api.Status {
	...
	if err == nil {
		err = rendered_version.RenderedVersionManager.Instance().Store(ctx, orgId, name, renderedVersion)
		if err != nil {
