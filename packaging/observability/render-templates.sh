...
render_templates() {
    ...
    require_bin /usr/bin/yq
    require_bin /usr/bin/envsubst
    ...
    while IFS='|' read -r env_var config_path default_value template_file output_file; do
        ...
        value=$(/usr/bin/yq e "$config_path" "$config_file" 2>/dev/null || echo "$default_value")
    ...
    for pair in "${RENDER_MAP[@]}"; do
        ...
        /usr/bin/envsubst < "$full_template_path" > "$output_file" || {
    ...
    for svc in "${!SYSTEMD_SERVICES[@]}"; do
        # Check if service unit file exists before trying to restart
        if /usr/bin/systemctl list-unit-files "$svc" >/dev/null 2>&1; then
            ...
            log "INFO" "Skipping $svc (service not found)"
        fi
    done
