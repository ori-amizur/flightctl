...
import (
	...
	"github.com/flightctl/flightctl/internal/api_server"
...
func main() {
	...
	listener, err := api_server.NewTLSListener(cfg.Service.Address, tlsConfig)
	if err != nil {
		log.Fatalf("creating listener: %s", err)
	}

	server := api_server.New(log, cfg, store, ca, listener)
