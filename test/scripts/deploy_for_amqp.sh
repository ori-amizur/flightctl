...
ONLY_DB=
NO_AUTH=
WITH_AMPQ="--set flightctl.api.enabled=false --set flightctl.worker.enabled=false"
...
while true; do
  case "$1" in
    -a|--only-db) ONLY_DB="--set flightctl.api.enabled=false --set flightctl.worker.enabled=false" ; shift ;;
...
helm repo update
helm install flightctl-rabbitmq bitnami/rabbitmq --values ./deploy/helm/flightctl_with_amqp/values.rabbitmq.yaml --namespace flightctl-internal

helm ${METHOD} --values ./deploy/helm/flightctl_with_amqp/values.kind.yaml --set flightctl.api.hostName=${IP}  ${ONLY_DB} ${NO_AUTH} ${DB_IMG} flightctl \
              ./deploy/helm/flightctl_with_amqp/ --kube-context kind-kind
...
if [ -z "$ONLY_DB" ]; then
  ...
  API_POD=$(kubectl get pod -n flightctl-external -L flightctl.service=flightctl-api --no-headers -o custom-columns=":metadata.name" --context kind-kind )
