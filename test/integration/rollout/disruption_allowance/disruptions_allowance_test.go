...
var _ = Describe("Rollout batch sequence test", func() {
	...
	setLabels := func(labels []map[string]string, numToSet []int) {
		...
		for i := 0; i != len(labels); i++ {
			for _, d := range devices.Items[offset : offset+numToSet[i]] {
				updateDeviceLabels(&d, labels[i])
			}
			offset += numToSet[i]
		}
	}
	...
	Context("Query fleets", func() {
		initTest := func(d *v1alpha1.DisruptionAllowance, numDevices int, annotateTv, annotateRenderedTv bool) {
			...
			if numDevices > 0 {
				...
				for _, d := range devices.Items {
					...
					Expect(err).ToNot(HaveOccurred())
					if annotateTv || annotateRenderedTv {
						annotations := make(map[string]string)
						if annotateTv {
							annotations[model.DeviceAnnotationTemplateVersion] = tvName
						}
						if annotateRenderedTv {
							annotations[model.DeviceAnnotationRenderedTemplateVersion] = tvName
						}
		...
		It("One fleet - 6 devices with matching fleet - with matching disruption allowance - with labels - without unavailable", func() {
			initTest(disruptionAllowance(nil, lo.ToPtr(1), lo.ToPtr([]string{"label-1", "label-2"})), 6, true, false)
			setLabels([]map[string]string{labels1, labels2}, []int{4, 2})
			reconciler := disruption_allowance.NewReconciler(storeInst, mockCallbackManager, log)
			mockCallbackManager.EXPECT().DeviceSourceUpdated(gomock.Any(), gomock.Any()).Times(3).Do(
				func(orgId uuid.UUID, name string) {
					device, err := storeInst.Device().Get(ctx, orgId, name)
					Expect(err).ToNot(HaveOccurred())
					Expect(lo.FromPtr(device.Metadata.Labels)).To(Equal(labels1))
				})
			mockCallbackManager.EXPECT().DeviceSourceUpdated(gomock.Any(), gomock.Any()).Do(
				func(orgId uuid.UUID, name string) {
					device, err := storeInst.Device().Get(ctx, orgId, name)
					Expect(err).ToNot(HaveOccurred())
					Expect(lo.FromPtr(device.Metadata.Labels)).To(Equal(labels2))
				})
			reconciler.Reconcile()
		})
	})
